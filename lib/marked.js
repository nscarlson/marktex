...
Lexer.prototype.token = function(src, top, bq) {
  ...
  while (src) {
    ...
    if (cap = this.rules.frontmatter.exec(src)) {
      src = src.substring(cap[0].length);
      this.tokens.push({
        type: 'frontmatter',
        text: cap[1]
...
Renderer.prototype.frontmatter = function(text) {
  if (typeof JSON === 'object' && typeof JSON.parse === 'function') {
    try {
      marked.lastFrontmatter = JSON.parse(text);
    } catch (e) {
      marked.lastFrontmatter = false;
    }
  } else {
    marked.lastFrontmatter = text;
  }
  return '';
};
...
Parser.prototype.parse = function(src) {
  ...
  this.inlineText = new InlineLexer([], merge({}, this.options, {renderer: new TextRenderer}));
